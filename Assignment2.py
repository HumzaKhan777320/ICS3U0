...
#half factors of its 1 parameter x

#x - is used as a parameter in the function myfunc and a variable
#to store the users input in the main code
...
while y:#creating while loop to run until the user enters done
    ...
    #taking input from the user and storing it in x
    ...
    try:#declaring try function
        if(n==0):#checking that n = 0 to make sure the input is valid(positive int)
            ...
            if(x>0):#checking that x is greater than 0(extra precaution)
                ...
                print("The most perimeter efficient dimensions of %d photos" %(x),end="")
